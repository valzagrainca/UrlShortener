...
export class UrlShortenerFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.urlShortenerService.errorOccurred.subscribe((res) => {
      if (res) {
  ...
  urlForm = new FormGroup({
    ...
    alias: new FormControl(null, Validators.maxLength(6)),
  ...
  shortUrlButtonClicked(): void {
    ...
    if (this.urlForm.valid) {
      this.urlShortenerService.posturlShorten(
        this.urlForm.value.fullURL,
        this.urlForm.value.alias,
        this.urlForm.value.expirationDate
      );
    }
  }
